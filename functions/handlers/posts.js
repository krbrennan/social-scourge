...
exports.getPost = (req, res) => {
  ...
  db.collection("/posts/")
    ...
    .then((data) => {
      ...
      return db
        .collection("comments")
        .where("comment.postId", "==", req.params.postId)
        .get();
    })
    .then((data) => {
      postInfo.comments = [];
  ...
  db.collection("/posts")
    ...
    .then((snapshot) => {
      ...
      if (postUser == requesterUsername) {
        ...
        res
          .status(500)
          .json({ error: "You cannot delete someone elses's post." });
      }
    })
    .catch((err) => {
              ...
              return db
                ...
                .then(() => {
                  ...
                  db.collection("posts")
                    ...
                    .get()
                    .then((data) => {
                      let newLikeCount = parseInt(
                        data._fieldsProto.likeCount.integerValue
                      );
                      postToLike
                        .update({
    ...
    .then((data) => {
      ...
      if (data._docs().length == 0) {
        return res.status(400).json({
          error:
            "post cannot be unliked if it was never liked in the first place!",
        });
      } else {
        // delete the like doc
...
exports.commentOnPost = (req, res) => {
  ...
  db.collection("comments")
    ...
    .then((data) => {
      ...
      let newCommentCount = parseInt(
        data._fieldsProto.commentCount.integerValue
      );
      return db
        .collection("posts")
        .doc(`${postId}`)
